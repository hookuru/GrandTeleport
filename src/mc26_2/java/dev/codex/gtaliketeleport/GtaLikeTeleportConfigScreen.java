...
final class GtaLikeTeleportConfigScreen extends Screen {
    ...
    private static final Identifier OVERWORLD_DIMENSION_ICON = Identifier.withDefaultNamespace("textures/block/grass_block_side.png");
    private static final Identifier NETHER_DIMENSION_ICON = Identifier.withDefaultNamespace("textures/block/netherrack.png");
    private static final Identifier END_DIMENSION_ICON = Identifier.withDefaultNamespace("textures/block/end_stone.png");
    ...
    private LockIconButton linkButton;
    ...
    private void initZoomWidgets() {
        ...
        this.linkButton = addRenderableWidget(new LockIconButton(linkRect.x, linkRect.y, button -> toggleLinked()));
    ...
    private DimensionIconButton addDimensionButton(String item, GtaLikeTeleportConfig.ZoomDimension dimension, ItemStack stack, Component label, Identifier texture) {
    ...
    public void extractBackground(GuiGraphicsExtractor context, int mouseX, int mouseY, float tickProgress) {
        ...
        context.outline(outer.x, outer.y, outer.width, outer.height, this.layoutDebugEnabled ? DEBUG_BORDER_COLOR : PANEL_BORDER_COLOR);
    }

    @Override
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float tickProgress) {
        super.extractRenderState(context, mouseX, mouseY, tickProgress);
        ...
        if (this.layoutDebugEnabled) {
            ...
            context.text(this.font, Component.translatable("gtalike_teleport.config.layout_debug_hint"), panel.x, panel.y + panel.height - 58, 0xFFFFFF55);
            extractLayoutDebugOverlays(context);
        }
    }

    private static void drawScaledText(GuiGraphicsExtractor context, net.minecraft.client.gui.Font font, Component text, int x, int y, int color, double scale) {
    ...
    private static void drawScaledCenteredText(GuiGraphicsExtractor context, net.minecraft.client.gui.Font font, Component text, int centerX, int y, int color, double scale) {
        if (Math.abs(scale - 1.0D) < 0.005D) {
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        normalizeValueEditorsForClick(event.x(), event.y());
        if (this.layoutDebugEnabled && isDebugControlAt(event.x(), event.y())) {
            return super.mouseClicked(event, doubleClick);
        }

        if (this.layoutDebugEnabled) {
            LayoutEditAction resizeAction = getResizeAction(this.selectedLayoutItem, event.x(), event.y());
            if (resizeAction != LayoutEditAction.NONE) {
                beginLayoutEdit(this.selectedLayoutItem, resizeAction, event.x(), event.y());
                return true;
            }

            String hitItem = findLayoutItemAt(event.x(), event.y());
            if (hitItem != null) {
                selectLayoutItem(hitItem);
                beginLayoutEdit(hitItem, LayoutEditAction.MOVE, event.x(), event.y());
                return true;
            }
        }
    ...
    public boolean mouseDragged(MouseButtonEvent event, double dragX, double dragY) {
        if (this.layoutEditAction != LayoutEditAction.NONE) {
            updateLayoutEdit(event.x(), event.y());
            return true;
        }
    ...
    private void resetCurrentPage() {
        ...
        if (this.currentPage == ConfigPage.OTHERS) {
            ...
            this.fallbackChunkFadeEnabled = true;
            ...
            GtaLikeTeleportConfig.setFallbackChunkFadeEnabled(true);
            updateOthersButtons();
        }
    }
    ...
    private void repositionWidgets() {
        ...
        if (this.zoomOutSlider != null) {
            ...
            this.zoomOutSlider.setRectangle(rect.width, rect.height, rect.x, rect.y);
        }
        if (this.zoomInSlider != null) {
            if (this.linked) {
                this.zoomInHeights = this.zoomOutHeights.clone();
            ...
            this.zoomInSlider.setRectangle(rect.width, rect.height, rect.x, rect.y);
        }
        if (this.linkButton != null) {
            LayoutRect rect = getItemRect(ITEM_LINK_BUTTON);
            this.linkButton.setRectangle(rect.width, rect.height, rect.x, rect.y);
        }
        repositionDimensionButton(this.overworldDimensionButton, ITEM_DIMENSION_OVERWORLD, GtaLikeTeleportConfig.ZoomDimension.OVERWORLD);
        repositionDimensionButton(this.netherDimensionButton, ITEM_DIMENSION_NETHER, GtaLikeTeleportConfig.ZoomDimension.NETHER);
        repositionDimensionButton(this.endDimensionButton, ITEM_DIMENSION_END, GtaLikeTeleportConfig.ZoomDimension.END);
        if (this.effectToggleButton != null) {
            ...
            this.effectToggleButton.setRectangle(rect.width, rect.height, rect.x, rect.y);
            this.effectToggleButton.setMessage(Component.empty());
        }
        if (this.movementToggleButton != null) {
            LayoutRect rect = getItemRect(ITEM_MOVEMENT_TOGGLE);
            this.movementToggleButton.setRectangle(rect.width, rect.height, rect.x, rect.y);
            this.movementToggleButton.setMessage(Component.empty());
        }
        if (this.crossDimensionTravelToggleButton != null) {
            LayoutRect rect = getItemRect(ITEM_CROSS_DIMENSION_TRAVEL_TOGGLE);
            this.crossDimensionTravelToggleButton.setRectangle(rect.width, rect.height, rect.x, rect.y);
            this.crossDimensionTravelToggleButton.setMessage(Component.empty());
        }
        if (this.soundModeToggleButton != null) {
            LayoutRect rect = getItemRect(ITEM_SOUND_MODE_TOGGLE);
            this.soundModeToggleButton.setRectangle(rect.width, rect.height, rect.x, rect.y);
            this.soundModeToggleButton.setMessage(Component.empty());
        }
        if (this.warpPlateToggleButton != null) {
            LayoutRect rect = getItemRect(ITEM_WARP_PLATE_TOGGLE);
            this.warpPlateToggleButton.setRectangle(rect.width, rect.height, rect.x, rect.y);
            this.warpPlateToggleButton.setMessage(Component.empty());
        }
        if (this.externalTeleportToggleButton != null) {
            LayoutRect rect = getItemRect(ITEM_EXTERNAL_TELEPORT_TOGGLE);
            this.externalTeleportToggleButton.setRectangle(rect.width, rect.height, rect.x, rect.y);
            this.externalTeleportToggleButton.active = isExternalTeleportToggleAvailable();
            this.externalTeleportToggleButton.setMessage(Component.empty());
        }
        if (this.minecraftSoundVolumeSlider != null) {
            LayoutRect rect = getItemRect(ITEM_MINECRAFT_VOLUME_SLIDER);
            this.minecraftSoundVolumeSlider.setRectangle(rect.width, rect.height, rect.x, rect.y);
        ...
        if (this.customSoundVolumeSlider != null) {
            ...
            this.customSoundVolumeSlider.setRectangle(rect.width, rect.height, rect.x, rect.y);
        ...
        if (this.zoomStageGlideSlider != null) {
            ...
            this.zoomStageGlideSlider.setRectangle(rect.width, rect.height, rect.x, rect.y);
        ...
        if (this.zoomStageGlideTicksEditBox != null) {
            ...
            this.zoomStageGlideTicksEditBox.setRectangle(rect.width, rect.height, rect.x, rect.y);
        ...
        if (this.zoomOutTicksEditBox != null) {
            ...
            this.zoomOutTicksEditBox.setRectangle(rect.width, rect.height, rect.x, rect.y);
        ...
        if (this.zoomInTicksEditBox != null) {
            ...
            this.zoomInTicksEditBox.setRectangle(rect.width, rect.height, rect.x, rect.y);
        ...
        if (this.bodyHeightSlider != null) {
            ...
            this.bodyHeightSlider.setRectangle(rect.width, rect.height, rect.x, rect.y);
        ...
        if (this.bodyGlideSlider != null) {
            ...
            this.bodyGlideSlider.setRectangle(rect.width, rect.height, rect.x, rect.y);
        ...
        if (this.bodyGlideTicksEditBox != null) {
            ...
            this.bodyGlideTicksEditBox.setRectangle(rect.width, rect.height, rect.x, rect.y);
        ...
        if (this.playerHideTicksEditBox != null) {
            ...
            this.playerHideTicksEditBox.setRectangle(rect.width, rect.height, rect.x, rect.y);
        ...
        if (this.prevPageButton != null) {
            ...
            this.prevPageButton.setRectangle(rect.width, rect.height, rect.x, rect.y);
            this.prevPageButton.setMessage(Component.empty());
        }
        if (this.nextPageButton != null) {
            LayoutRect rect = getItemRect(ITEM_NEXT_PAGE_BUTTON);
            this.nextPageButton.setRectangle(rect.width, rect.height, rect.x, rect.y);
        ...
        if (this.layoutDebugButton != null) {
            int buttonWidth = 90;
            this.layoutDebugButton.setRectangle(buttonWidth, 20, Math.max(6, this.width - buttonWidth - 6), 6);
        ...
        if (this.resetButton != null) {
            ...
            this.resetButton.setRectangle(rect.width, rect.height, rect.x, rect.y);
        }
        if (this.doneButton != null) {
            LayoutRect rect = getItemRect(ITEM_DONE_BUTTON);
            this.doneButton.setRectangle(rect.width, rect.height, rect.x, rect.y);
        }
    ...
    private ToolbarCursor placeToolbarButton(Button button, Component label, ToolbarCursor cursor, int width, int rightLimit) {
        ...
        button.setRectangle(width, DEBUG_TOOLBAR_BUTTON_HEIGHT, buttonX, buttonY);
    ...
    private void extractLayoutGuides(GuiGraphicsExtractor context) {
        LayoutRect panel = getContentRect();
    ...
    private static void drawVerticalGuide(GuiGraphicsExtractor context, int x, int y1, int y2, int color) {
        context.fill(x, y1, x + 1, y2, color);
    }

    private static void drawHorizontalGuide(GuiGraphicsExtractor context, int y, int x1, int x2, int color) {
        context.fill(x1, y, x2, y + 1, color);
    }
    private void extractLayoutDebugOverlays(GuiGraphicsExtractor context) {
        for (String item : getVisibleLayoutItems()) {
            LayoutRect rect = getSelectableRect(item);
            boolean selected = item.equals(this.selectedLayoutItem);
            int color = selected ? DEBUG_BORDER_COLOR : DEBUG_ITEM_BORDER_COLOR;
            context.outline(rect.x, rect.y, rect.width, rect.height, color);
        }
        ...
        if (this.selectedLayoutItem != null && isVisibleLayoutItem(this.selectedLayoutItem)) {
            ...
            context.outline(rect.right() - 9, rect.bottom() - 9, 11, 11, 0xFFFFFFFF);
        }
    }
    ...
    private void refreshLayoutValueEditor() {
        ...
        if (!visible) {
            ...
            if (this.applyLayoutValueButton != null) {
                this.applyLayoutValueButton.setRectangle(1, 1, -1000, -1000);
            }
            return;
        }
    ...
    private void repositionLayoutValueEditor(ToolbarCursor cursor, int rightLimit) {
        ...
        if (editorX + editorWidth + DEBUG_TOOLBAR_GAP + applyWidth > rightLimit) {
            ...
            this.layoutValueEditBox.setRectangle(editorWidth, DEBUG_TOOLBAR_BUTTON_HEIGHT, editorX, editorY);
            if (this.applyLayoutValueButton != null) {
                this.applyLayoutValueButton.setRectangle(applyWidth, DEBUG_TOOLBAR_BUTTON_HEIGHT, editorX, editorY + DEBUG_TOOLBAR_ROW_STEP);
            }
            return;
        }
        ...
        this.layoutValueEditBox.setRectangle(editorWidth, DEBUG_TOOLBAR_BUTTON_HEIGHT, editorX, editorY);
        if (this.applyLayoutValueButton != null) {
            this.applyLayoutValueButton.setRectangle(applyWidth, DEBUG_TOOLBAR_BUTTON_HEIGHT, editorX + editorWidth + DEBUG_TOOLBAR_GAP, editorY);
        }
    }
    ...
    private void repositionTextEditor() {
        ...
        this.textEditBox.setRectangle(editorWidth, editorHeight, editorX, editorY);
    }
    ...
    private void drawButtonText(GuiGraphicsExtractor context, String item) {
        LayoutRect rect = getItemRect(item);
    ...
    private static final class ScaledEditBox extends EditBox {
        ...
        @Override
        public void extractWidgetRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float tickProgress) {
            int x = getX();
            int y = getY();
            int width = getWidth();
        ...
        public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
            ...
            int cursor = getCursorPositionForMouse(event.x());
            setFocused(true);
        ...
        @Override
        public boolean mouseDragged(MouseButtonEvent event, double dragX, double dragY) {
            if (!this.selectingText || event.button() != 0) {
                return false;
            }
        ...
        private void drawSelection(GuiGraphicsExtractor context, String value, int textX, int textY, int textHeight, double scale) {
    ...
    private static final class DimensionIconButton extends AbstractWidget {
        ...
        DimensionIconButton(int x, int y, int width, int height, ItemStack icon, Component label, Identifier texture, Runnable onPress) {
        ...
        @Override
        protected void extractWidgetRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float tickProgress) {
            int x = getX();
            int y = getY();
            ...
            context.outline(x, y, getWidth(), getHeight(), borderColor);
            ...
            if (!this.icon.isEmpty()) {
                context.pose().pushMatrix();
                context.pose().translate((float) iconX, (float) iconY);
                context.pose().scale((float) scale);
                context.item(this.icon, 0, 0);
                context.pose().popMatrix();
            } else {
        ...
        private static void drawTextureIcon(GuiGraphicsExtractor context, int x, int y, double scale, Identifier texture) {
            int size = Math.max(4, (int) Math.round(16.0D * scale));
            context.blit(RenderPipelines.GUI_TEXTURED, texture, x, y, 0.0F, 0.0F, size, size, 16, 16, 16, 16);
        }
    ...
    private static final class SingleValueSlider extends AbstractWidget {
        ...
        protected void extractWidgetRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float tickProgress) {
            ...
            context.fill(x, y, right, bottom, this.active ? 0xAA151515 : 0x88101010);
            context.outline(x, y, getWidth(), getHeight(), borderColor);
            drawScaledText(context, Minecraft.getInstance().font, getMessage(), x + scaled(8, scale), y + scaled(7, scale), textColor, scale);
            Component formatted = Component.literal(formatValue());
            int valueWidth = (int) Math.round(Minecraft.getInstance().font.width(formatted) * scale);
            drawScaledText(context, Minecraft.getInstance().font, formatted, right - valueWidth - scaled(8, scale), y + scaled(7, scale), valueColor, scale);
    ...
    private static final class StageHeightSlider extends AbstractWidget {
        ...
        protected void extractWidgetRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float tickProgress) {
            ...
            context.outline(x, y, getWidth(), getHeight(), borderColor);
            drawScaledText(context, Minecraft.getInstance().font, getMessage(), x + scaled(8, scale), y + scaled(7, scale), textColor, scale);
            int valuesWidth = (int) Math.round(Minecraft.getInstance().font.width(formatValues()) * scale);
            drawScaledText(context, Minecraft.getInstance().font, formatValues(), right - valuesWidth - scaled(8, scale), y + scaled(7, scale), valueColor, scale);
        ...
        public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
            ...
            this.activeHandle = nearestHandle(event.x());
            updateValueFromMouse(event.x());
            playDownSound(Minecraft.getInstance().getSoundManager());
            return true;
        }
