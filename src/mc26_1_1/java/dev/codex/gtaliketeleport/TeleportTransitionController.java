...
public final class TeleportTransitionController {
    ...
    private static final int VANILLA_TERRAIN_FADE_IN_EXTRA_TICKS = 10;
    ...
    public static float getFallbackTerrainSectionVisibility(double centerX, double centerY, double centerZ) {
        ...
        int fadeInDuration = getVanillaTerrainFadeInDurationTicks();
        ...
        if (frameTick >= fadeInStart && frameTick < fadeInEnd) {
            float progress = smoothStep((frameTick - fadeInStart) / fadeInDuration);
    ...
    public static float getShaderScreenMaskIntensity(float tickProgress) {
        if (!SHADER_SCREEN_MASK_ENABLED || !shouldHardCutFallbackTerrain() || cameraReleased) {
            return 0.0F;
        }
    ...
    private static int getCommandSendTick() {
        ...
        if (plannedTargetFeetStable && plannedTargetFeet != null) {
            int sendDelay = Math.min(travelTicks / 2, PRE_TRAVEL_WAIT_TICKS);
            return getTravelStartTick() + sendDelay;
        }
    ...
    private static int getVanillaTerrainFadeInEndTick() {
        return getVanillaTerrainFadeInStartTick() + getVanillaTerrainFadeInDurationTicks();
    }

    private static int getVanillaTerrainFadeInDurationTicks() {
        return FALLBACK_TERRAIN_SWEEP_TICKS + VANILLA_TERRAIN_FADE_IN_EXTRA_TICKS;
    }
    ...
    private static float getDistanceOrderTowardPoint(double sectionCenterX, double sectionCenterZ, Vec3 centerFeet, Vec3 directionFeet) {
        ...
        if (centerFeet == null || directionFeet == null) {
            return 0.5F;
        }

        double directionX = directionFeet.x - centerFeet.x;
        double directionZ = directionFeet.z - centerFeet.z;
        ...
        double offsetX = sectionCenterX - centerFeet.x;
        double offsetZ = sectionCenterZ - centerFeet.z;
        double projectedTowardDirection = offsetX * unitX + offsetZ * unitZ;
        return (float) Mth.clamp(0.5D - projectedTowardDirection / Math.max(1.0D, radius * 2.0D), 0.0D, 1.0D);
    }
